"""Adpatar o 107 - Módulos
- Criar uma função adicional moeda() que consiga mostrar os valores monetários formatados"""

# import moeda # importando o módulo de funções que criei
from ex108 import moeda # Faz a mesma importação só em 'moeda'

preco = float(input('Digite o preço (R$): '))
print(f'A metade de {moeda.moeda(preco)} é {moeda.moeda(moeda.metade(preco))}.')
print(f'O dobro de {moeda.moeda(preco)} é {moeda.moeda(moeda.dobro(preco))}.')
print(f'Aumentando 10%, temos: {moeda.moeda(moeda.aumentar(preco, 10))}.')
print(f'Reduzindo 13%, temos: {moeda.moeda(moeda.diminuir(preco, 13))}.')
